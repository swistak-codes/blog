...
import { porownanieTestowPierwszosci } from '../../_posts/content/20220316-porownanie-szybkich-testow-pierwszosci';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{porownanieTestowPierwszosci.meta.firstParagraph}</p>
      <porownanieTestowPierwszosci.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    porownanieTestowPierwszosci.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    porownanieTestowPierwszosci.meta,
    prevNextSlugs,
    similarPosts,
  );
