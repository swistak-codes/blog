...
import { mierzenieCzasuWykonania } from '../../_posts/content/20240110-mierzenie-czasu-wykonania';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{mierzenieCzasuWykonania.meta.firstParagraph}</p>
      <mierzenieCzasuWykonania.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(mierzenieCzasuWykonania.meta.publishTime);
  const modifiedMeta = convertMetadata(
    mierzenieCzasuWykonania.meta,
    prevNextSlugs,
    similarPosts,
  );
