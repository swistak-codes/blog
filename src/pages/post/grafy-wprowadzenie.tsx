...
import { grafyWprowadzenie } from '../../_posts/content/20220601-grafy-wprowadzenie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{grafyWprowadzenie.meta.firstParagraph}</p>
      <grafyWprowadzenie.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(grafyWprowadzenie.meta.publishTime);
  const modifiedMeta = convertMetadata(
    grafyWprowadzenie.meta,
    prevNextSlugs,
    similarPosts,
  );
