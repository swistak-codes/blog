...
import { tekstowyZapisDanych } from '../../_posts/content/20201104-tekstowy-zapis-danych-cyfrowych';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{tekstowyZapisDanych.meta.firstParagraph}</p>
      <tekstowyZapisDanych.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(tekstowyZapisDanych.meta.publishTime);
  const modifiedMeta = convertMetadata(
    tekstowyZapisDanych.meta,
    prevNextSlugs,
    similarPosts,
  );
