...
import { liczbyPierwszeProsteSposobyZnajdowanie } from '../../_posts/content/20220202-proste-sposoby-na-znajdowanie-liczb-pierwszych';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{liczbyPierwszeProsteSposobyZnajdowanie.meta.firstParagraph}</p>
      <liczbyPierwszeProsteSposobyZnajdowanie.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    liczbyPierwszeProsteSposobyZnajdowanie.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    liczbyPierwszeProsteSposobyZnajdowanie.meta,
    prevNextSlugs,
    similarPosts,
  );
