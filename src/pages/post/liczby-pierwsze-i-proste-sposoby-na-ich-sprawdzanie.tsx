...
import { liczbyPierwszeProsteSposoby } from '../../_posts/content/20220119-liczby-pierwsze-i-proste-sposoby-na-ich-sprawdzanie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{liczbyPierwszeProsteSposoby.meta.firstParagraph}</p>
      <liczbyPierwszeProsteSposoby.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    liczbyPierwszeProsteSposoby.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    liczbyPierwszeProsteSposoby.meta,
    prevNextSlugs,
    similarPosts,
  );
