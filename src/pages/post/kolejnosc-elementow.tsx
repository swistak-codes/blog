...
import { kolejnoscElementow } from '../../_posts/content/20241218-kolejnosc-elementow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{kolejnoscElementow.meta.firstParagraph}</p>
      <kolejnoscElementow.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(kolejnoscElementow.meta.publishTime);
  const modifiedMeta = convertMetadata(
    kolejnoscElementow.meta,
    prevNextSlugs,
    similarPosts,
  );
