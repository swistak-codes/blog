...
import { przechodzeniePoGrafie } from '../../_posts/content/20220810-przechodzenie-po-grafie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{przechodzeniePoGrafie.meta.firstParagraph}</p>
      <przechodzeniePoGrafie.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(przechodzeniePoGrafie.meta.publishTime);
  const modifiedMeta = convertMetadata(
    przechodzeniePoGrafie.meta,
    prevNextSlugs,
    similarPosts,
  );
