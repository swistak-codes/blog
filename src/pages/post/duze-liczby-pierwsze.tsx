...
import { duzeLiczbyPierwsze } from '../../_posts/content/20220216-duze-liczby-pierwsze';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{duzeLiczbyPierwsze.meta.firstParagraph}</p>
      <duzeLiczbyPierwsze.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(duzeLiczbyPierwsze.meta.publishTime);
  const modifiedMeta = convertMetadata(
    duzeLiczbyPierwsze.meta,
    prevNextSlugs,
    similarPosts,
  );
