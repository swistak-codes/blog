...
import { szybkieSzukanieSciezek } from '../../_posts/content/20220928-szybkie-wyszukiwanie-sciezek';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{szybkieSzukanieSciezek.meta.firstParagraph}</p>
      <szybkieSzukanieSciezek.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(szybkieSzukanieSciezek.meta.publishTime);
  const modifiedMeta = convertMetadata(
    szybkieSzukanieSciezek.meta,
    prevNextSlugs,
    similarPosts,
  );
