...
import { jakKomputerPrzechowujeDate } from '../../_posts/content/20210707-jak-komputer-przechowuje-date-i-skad-zna-aktualna';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{jakKomputerPrzechowujeDate.meta.firstParagraph}</p>
      <jakKomputerPrzechowujeDate.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    jakKomputerPrzechowujeDate.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    jakKomputerPrzechowujeDate.meta,
    prevNextSlugs,
    similarPosts,
  );
