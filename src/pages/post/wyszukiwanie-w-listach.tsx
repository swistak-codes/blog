...
import { wyszukiwanieWListach } from '../../_posts/content/20201111-wyszukiwanie-w-listach';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{wyszukiwanieWListach.meta.firstParagraph}</p>
      <wyszukiwanieWListach.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(wyszukiwanieWListach.meta.publishTime);
  const modifiedMeta = convertMetadata(
    wyszukiwanieWListach.meta,
    prevNextSlugs,
    similarPosts,
  );
