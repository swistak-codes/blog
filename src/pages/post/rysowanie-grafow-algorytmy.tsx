...
import { rysowanieGrafow } from '../../_posts/content/20221012-rysowanie-grafow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{rysowanieGrafow.meta.firstParagraph}</p>
      <rysowanieGrafow.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(rysowanieGrafow.meta.publishTime);
  const modifiedMeta = convertMetadata(
    rysowanieGrafow.meta,
    prevNextSlugs,
    similarPosts,
  );
