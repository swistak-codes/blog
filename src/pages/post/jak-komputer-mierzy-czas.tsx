...
import { jakKomputerMierzyCzas } from '../../_posts/content/20210623-jak-komputer-mierzy-czas';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{jakKomputerMierzyCzas.meta.firstParagraph}</p>
      <jakKomputerMierzyCzas.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(jakKomputerMierzyCzas.meta.publishTime);
  const modifiedMeta = convertMetadata(
    jakKomputerMierzyCzas.meta,
    prevNextSlugs,
    similarPosts,
  );
