...
import { maturaProbna2023 } from '../../_posts/content/20240410-matura-probna-2023';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{maturaProbna2023.meta.firstParagraph}</p>
      <maturaProbna2023.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(maturaProbna2023.meta.publishTime);
  const modifiedMeta = convertMetadata(
    maturaProbna2023.meta,
    prevNextSlugs,
    similarPosts,
  );
