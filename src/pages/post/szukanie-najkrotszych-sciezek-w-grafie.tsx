...
import { szukanieSciezekGraf } from '../../_posts/content/20220916-szukanie-najkrotszych-sciezek-w-grafie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{szukanieSciezekGraf.meta.firstParagraph}</p>
      <szukanieSciezekGraf.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(szukanieSciezekGraf.meta.publishTime);
  const modifiedMeta = convertMetadata(
    szukanieSciezekGraf.meta,
    prevNextSlugs,
    similarPosts,
  );
