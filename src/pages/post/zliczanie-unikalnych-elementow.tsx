...
import { zliczanieUnikalnych } from '../../_posts/content/20240717-zliczanie-unikalnych-elementow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{zliczanieUnikalnych.meta.firstParagraph}</p>
      <zliczanieUnikalnych.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(zliczanieUnikalnych.meta.publishTime);
  const modifiedMeta = convertMetadata(
    zliczanieUnikalnych.meta,
    prevNextSlugs,
    similarPosts,
  );
