...
import { przeksztalcenia3D } from '../../_posts/content/20210609-przeksztalcenia-grafiki-3d';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{przeksztalcenia3D.meta.firstParagraph}</p>
      <przeksztalcenia3D.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(przeksztalcenia3D.meta.publishTime);
  const modifiedMeta = convertMetadata(
    przeksztalcenia3D.meta,
    prevNextSlugs,
    similarPosts,
  );
