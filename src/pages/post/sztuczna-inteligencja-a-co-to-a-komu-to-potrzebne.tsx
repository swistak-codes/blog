...
import { sztucznaInteligencja } from '../../_posts/content/20200622-sztuczna-inteligencja-a-co-to-a-komu-to-potrzebne';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{sztucznaInteligencja.meta.firstParagraph}</p>
      <sztucznaInteligencja.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(sztucznaInteligencja.meta.publishTime);
  const modifiedMeta = convertMetadata(
    sztucznaInteligencja.meta,
    prevNextSlugs,
    similarPosts,
  );
