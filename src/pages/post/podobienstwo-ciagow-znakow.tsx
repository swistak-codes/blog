...
import { podobienstwoStringi } from '../../_posts/content/20230913-podobienstwo-ciagow-znakow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{podobienstwoStringi.meta.firstParagraph}</p>
      <podobienstwoStringi.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(podobienstwoStringi.meta.publishTime);
  const modifiedMeta = convertMetadata(
    podobienstwoStringi.meta,
    prevNextSlugs,
    similarPosts,
  );
