...
import { operacjeNaBarwach } from '../../_posts/content/20211208-podstawowe-operacje-na-barwach';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{operacjeNaBarwach.meta.firstParagraph}</p>
      <operacjeNaBarwach.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(operacjeNaBarwach.meta.publishTime);
  const modifiedMeta = convertMetadata(
    operacjeNaBarwach.meta,
    prevNextSlugs,
    similarPosts,
  );
