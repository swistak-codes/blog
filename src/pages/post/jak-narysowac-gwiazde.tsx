...
import { jakRysowacGwiazdy } from '../../_posts/content/20241204-gwiazdy';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{jakRysowacGwiazdy.meta.firstParagraph}</p>
      <jakRysowacGwiazdy.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(jakRysowacGwiazdy.meta.publishTime);
  const modifiedMeta = convertMetadata(
    jakRysowacGwiazdy.meta,
    prevNextSlugs,
    similarPosts,
  );
