...
import { jakKomputerZapisujeDzwiek } from '../../_posts/content/20210511-jak-komputer-zapisuje-dzwiek';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{jakKomputerZapisujeDzwiek.meta.firstParagraph}</p>
      <jakKomputerZapisujeDzwiek.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    jakKomputerZapisujeDzwiek.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    jakKomputerZapisujeDzwiek.meta,
    prevNextSlugs,
    similarPosts,
  );
