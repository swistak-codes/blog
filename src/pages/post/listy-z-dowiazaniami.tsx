...
import { listyZDowiazaniami } from '../../_posts/content/20200522-listy-z-dowiazaniami';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{listyZDowiazaniami.meta.firstParagraph}</p>
      <listyZDowiazaniami.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(listyZDowiazaniami.meta.publishTime);
  const modifiedMeta = convertMetadata(
    listyZDowiazaniami.meta,
    prevNextSlugs,
    similarPosts,
  );
