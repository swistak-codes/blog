...
import { komputerWKomputerze } from '../../_posts/content/20210428-komputer-w-komputerze-czyli-emulacja-wirtualizacja-i-konteneryzacja';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{komputerWKomputerze.meta.firstParagraph}</p>
      <komputerWKomputerze.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(komputerWKomputerze.meta.publishTime);
  const modifiedMeta = convertMetadata(
    komputerWKomputerze.meta,
    prevNextSlugs,
    similarPosts,
  );
