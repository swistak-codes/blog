...
import { sposobyZapisywaniaLiczb } from '../../_posts/content/20200930-sposoby-zapisywania-liczb-przez-komputery';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{sposobyZapisywaniaLiczb.meta.firstParagraph}</p>
      <sposobyZapisywaniaLiczb.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(sposobyZapisywaniaLiczb.meta.publishTime);
  const modifiedMeta = convertMetadata(
    sposobyZapisywaniaLiczb.meta,
    prevNextSlugs,
    similarPosts,
  );
