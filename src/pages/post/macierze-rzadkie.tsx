...
import { macierzeRzadkie } from '../../_posts/content/20240911-macierze-rzadkie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{macierzeRzadkie.meta.firstParagraph}</p>
      <macierzeRzadkie.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(macierzeRzadkie.meta.publishTime);
  const modifiedMeta = convertMetadata(
    macierzeRzadkie.meta,
    prevNextSlugs,
    similarPosts,
  );
