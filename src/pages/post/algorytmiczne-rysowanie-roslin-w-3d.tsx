...
import { rysowanieRoslin3d } from '../../_posts/content/20220105-algorytmiczne-rysowanie-roslin-w-3d';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{rysowanieRoslin3d.meta.firstParagraph}</p>
      <rysowanieRoslin3d.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(rysowanieRoslin3d.meta.publishTime);
  const modifiedMeta = convertMetadata(
    rysowanieRoslin3d.meta,
    prevNextSlugs,
    similarPosts,
  );
