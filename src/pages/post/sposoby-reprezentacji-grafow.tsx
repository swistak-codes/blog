...
import { sposobyReprezentacjiGrafow } from '../../_posts/content/20220615-sposoby-reprezentacji-grafow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{sposobyReprezentacjiGrafow.meta.firstParagraph}</p>
      <sposobyReprezentacjiGrafow.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    sposobyReprezentacjiGrafow.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    sposobyReprezentacjiGrafow.meta,
    prevNextSlugs,
    similarPosts,
  );
