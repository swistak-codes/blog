...
import { problemKomiwojazeraHeurystyki } from '../../_posts/content/20241009-problem-komiwojazera-heurystyki';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{problemKomiwojazeraHeurystyki.meta.firstParagraph}</p>
      <problemKomiwojazeraHeurystyki.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    problemKomiwojazeraHeurystyki.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    problemKomiwojazeraHeurystyki.meta,
    prevNextSlugs,
    similarPosts,
  );
