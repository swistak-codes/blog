...
import { pierwiastkowanie } from '../../_posts/content/20230125-pierwiastkowanie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{pierwiastkowanie.meta.firstParagraph}</p>
      <pierwiastkowanie.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(pierwiastkowanie.meta.publishTime);
  const modifiedMeta = convertMetadata(
    pierwiastkowanie.meta,
    prevNextSlugs,
    similarPosts,
  );
