...
import { praktykaPrzechodzeniePoGrafie } from '../../_posts/content/20220831-praktyczne-zastosowania-przechodzenia-po-grafie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{praktykaPrzechodzeniePoGrafie.meta.firstParagraph}</p>
      <praktykaPrzechodzeniePoGrafie.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    praktykaPrzechodzeniePoGrafie.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    praktykaPrzechodzeniePoGrafie.meta,
    prevNextSlugs,
    similarPosts,
  );
