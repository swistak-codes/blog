...
import { kompresjaObrazow } from '../../_posts/content/20240522-kompresja-obrazow';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{kompresjaObrazow.meta.firstParagraph}</p>
      <kompresjaObrazow.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(kompresjaObrazow.meta.publishTime);
  const modifiedMeta = convertMetadata(
    kompresjaObrazow.meta,
    prevNextSlugs,
    similarPosts,
  );
