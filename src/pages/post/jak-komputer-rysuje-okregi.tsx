...
import { jakRysujeOkregi } from '../../_posts/content/20210317-jak-komputer-rysuje-okregi';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{jakRysujeOkregi.meta.firstParagraph}</p>
      <jakRysujeOkregi.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(jakRysujeOkregi.meta.publishTime);
  const modifiedMeta = convertMetadata(
    jakRysujeOkregi.meta,
    prevNextSlugs,
    similarPosts,
  );
