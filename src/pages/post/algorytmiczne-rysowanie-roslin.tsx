...
import { rysowanieRoslin } from '../../_posts/content/20211222-algorytmiczne-rysowanie-roslin';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{rysowanieRoslin.meta.firstParagraph}</p>
      <rysowanieRoslin.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(rysowanieRoslin.meta.publishTime);
  const modifiedMeta = convertMetadata(
    rysowanieRoslin.meta,
    prevNextSlugs,
    similarPosts,
  );
