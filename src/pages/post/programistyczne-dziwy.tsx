...
import { programistyczneDziwactwa } from '../../_posts/content/20240703-programistyczne-dziwactwa';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{programistyczneDziwactwa.meta.firstParagraph}</p>
      <programistyczneDziwactwa.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    programistyczneDziwactwa.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    programistyczneDziwactwa.meta,
    prevNextSlugs,
    similarPosts,
  );
