...
import { nieliczbyJakoLiczby } from '../../_posts/content/20201014-nie-liczby-jako-liczby-czyli-zapis-danych-cyfrowych';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{nieliczbyJakoLiczby.meta.firstParagraph}</p>
      <nieliczbyJakoLiczby.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(nieliczbyJakoLiczby.meta.publishTime);
  const modifiedMeta = convertMetadata(
    nieliczbyJakoLiczby.meta,
    prevNextSlugs,
    similarPosts,
  );
