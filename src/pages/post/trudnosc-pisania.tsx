...
import { trudnoscPisania } from '../../_posts/content/20251112-trudnosc-pisania';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{trudnoscPisania.meta.firstParagraph}</p>
      <trudnoscPisania.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(trudnoscPisania.meta.publishTime);
  const modifiedMeta = convertMetadata(
    trudnoscPisania.meta,
    prevNextSlugs,
    similarPosts,
  );
