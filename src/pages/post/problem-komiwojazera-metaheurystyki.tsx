...
import { problemKomiwojazeraMetaheurystyki } from '../../_posts/content/20241030-problem-komiwojazera-metaheurystyki';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{problemKomiwojazeraMetaheurystyki.meta.firstParagraph}</p>
      <problemKomiwojazeraMetaheurystyki.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    problemKomiwojazeraMetaheurystyki.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    problemKomiwojazeraMetaheurystyki.meta,
    prevNextSlugs,
    similarPosts,
  );
