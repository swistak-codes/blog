...
import { resztaZDzielenia } from '../../_posts/content/20210901-dziwny-przypadek-reszty-z-dzielenia';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{resztaZDzielenia.meta.firstParagraph}</p>
      <resztaZDzielenia.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(resztaZDzielenia.meta.publishTime);
  const modifiedMeta = convertMetadata(
    resztaZDzielenia.meta,
    prevNextSlugs,
    similarPosts,
  );
