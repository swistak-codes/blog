...
import { systemyLiczbowe } from '../../_posts/content/20201021-systemy-liczbowe-uzupelnienie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{systemyLiczbowe.meta.firstParagraph}</p>
      <systemyLiczbowe.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(systemyLiczbowe.meta.publishTime);
  const modifiedMeta = convertMetadata(
    systemyLiczbowe.meta,
    prevNextSlugs,
    similarPosts,
  );
