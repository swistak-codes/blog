...
import { problemKomiwojazera } from '../../_posts/content/20240925-problem-komiwojazera';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{problemKomiwojazera.meta.firstParagraph}</p>
      <problemKomiwojazera.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(problemKomiwojazera.meta.publishTime);
  const modifiedMeta = convertMetadata(
    problemKomiwojazera.meta,
    prevNextSlugs,
    similarPosts,
  );
