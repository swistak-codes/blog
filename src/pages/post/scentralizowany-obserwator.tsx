...
import { scentralizowanyObserwator } from '../../_posts/content/20231018-scentralizowany-obserwator';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{scentralizowanyObserwator.meta.firstParagraph}</p>
      <scentralizowanyObserwator.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    scentralizowanyObserwator.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    scentralizowanyObserwator.meta,
    prevNextSlugs,
    similarPosts,
  );
