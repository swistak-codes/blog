...
import { szybkieSzukanieLiczbPierwszych } from '../../_posts/content/20220302-szybkie-szukanie-duzych-liczb-pierwszych';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{szybkieSzukanieLiczbPierwszych.meta.firstParagraph}</p>
      <szybkieSzukanieLiczbPierwszych.Component />
    </Post>
  );
}
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(
    szybkieSzukanieLiczbPierwszych.meta.publishTime,
  );
  const modifiedMeta = convertMetadata(
    szybkieSzukanieLiczbPierwszych.meta,
    prevNextSlugs,
    similarPosts,
  );
