...
import { maturaProbna2022 } from '../../_posts/content/20230419-matura-probna-2022';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{maturaProbna2022.meta.firstParagraph}</p>
      <maturaProbna2022.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(maturaProbna2022.meta.publishTime);
  const modifiedMeta = convertMetadata(
    maturaProbna2022.meta,
    prevNextSlugs,
    similarPosts,
  );
