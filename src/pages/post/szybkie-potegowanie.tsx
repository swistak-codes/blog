...
import { szybkiePotegowanie } from '../../_posts/content/20230111-podstawy-algorytmiki-szybkie-potegowanie';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{szybkiePotegowanie.meta.firstParagraph}</p>
      <szybkiePotegowanie.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(szybkiePotegowanie.meta.publishTime);
  const modifiedMeta = convertMetadata(
    szybkiePotegowanie.meta,
    prevNextSlugs,
    similarPosts,
  );
