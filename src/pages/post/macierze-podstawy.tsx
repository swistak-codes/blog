...
import { macierzePodstawy } from '../../_posts/content/20240807-macierze-podstawy';
...
export function Article({
  ...
  return (
    <Post metadata={modifiedMeta} categoryMap={categoryMap} tagsMap={tagsMap}>
      ...
      <p>{macierzePodstawy.meta.firstParagraph}</p>
      <macierzePodstawy.Component />
    </Post>
  );
}

export const getStaticProps: GetStaticProps<Props> = async () => {
  const dateLink = getDatePathForDate(macierzePodstawy.meta.publishTime);
  const modifiedMeta = convertMetadata(
    macierzePodstawy.meta,
    prevNextSlugs,
    similarPosts,
  );
