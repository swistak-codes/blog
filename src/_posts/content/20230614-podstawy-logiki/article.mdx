...
Przejdźmy teraz na język logiki matematycznej. Tutaj zdania oznaczamy zwykle symbolami $$p,q,r,s$$, natomiast wartości: *prawda* to 1, *fałsz* to 0. Można by powiedzieć, że operujemy [systemem binarnym](/post/matematyka-zero-jedynkowa), co z punktu widzenia informatyki jest dość istotne.
...
Jeśli jesteś ciekaw(a), dlaczego negacja w postaci operatora bitowego zamienia liczby na ujemne mniejsze o 1, to wynika to z tego, że w pokazanym tu języku liczby są zapisane ze znakiem, a dokładniej [kodowaniem U2](/post/sposoby-zapisywania-liczb-przez-komputery/#liczby-ca%C5%82kowite). Gdybyśmy korzystali z typów całkowitoliczbowych bez znaku, wyglądałoby to następująco (typ 8-bitowy, taki jak `unsigned char` z C):
...
Przykład, jak takie sprawdzenie parzystości wygląda w praktyce, pokazałem w artykule [o szybkim potęgowaniu](/post/szybkie-potegowanie/#kod-według-definicji-binarnej), gdzie zapraszam.

### Kryptografia

Przejdźmy teraz do XOR. O przykładowym jego zastosowaniu przy obliczaniu kodów CRC napisałem [w artykule o sumach kontrolnych](/post/sumy-kontrolne), ale zobaczmy jeszcze inne zastosowania. Szczególnie te związane z kryptografią, bo tutaj w niemal każdym bardziej zaawansowanym algorytmie, czy to szyfrowania, czy funkcji haszujących, znajdziemy tę operację, między innymi dlatego, że jest odwracalna, co pokazałem wcześniej na przykładzie w kodzie.
...
Możesz go przetestować na [OnlineGDB](https://onlinegdb.com/fhq9B3Pe1).
