...
export const meta = {
  ...
  coverCaption: (
    <>
      (zdjęcie na okładce pochodzi z serwisu <a href="https://pixabay.com/illustrations/bird-parrot-colorful-animal-3769459/" target="_blank" rel="noreferrer">Pixabay</a>)
    </>
  ),
};
...
Dodatkowo, jeśli nie miałeś(-aś) okazji czytać mojego [artykułu na temat barw i ich reprezentacji w komputerze](/post/jak-komputer-zapisuje-kolory), nadrób tę zaległość, bo poniższy tekst można traktować jako kontynuację tamtego artykułu. Nawet jeśli takie pojęcia, jak RGB, głębia kolorów czy dithering nie są Ci obce, to warto przypomnieć sobie nieco teorii.
