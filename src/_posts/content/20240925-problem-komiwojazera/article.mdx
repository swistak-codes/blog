...
We wstępie napisałem, że problem komiwojażera to powrót do tematyki [grafów](/category/grafy), której do tej pory poświęciłem 8 artykułów. Jeśli nie wiesz, czym są grafy, albo potrzebujesz powtórki z teorii, zapraszam do artykułu *[Grafy — wprowadzenie](/post/grafy-wprowadzenie)*.
...
- Cykl Hamiltona — jest to zamknięta ścieżka Hamiltona.
    ...
    - Temat cykli i ścieżek Hamiltona szerzej poruszyłem w artykule *[Problem skoczka szachowego](/post/problem-skoczka-szachowego)*.
...
Jest to nic innego jak silnia z $$n$$. Silnię omawiałem m.in. przy okazji [rekurencji](/post/rekurencja-co-to-jest) albo obliczania [symbolu Newtona](/post/symbol-newtona), więc jeśli nie wiesz, jak ją obliczać, zapraszam tam.
...
Całość kodu, wraz z odczytem przykładowych danych (att48 ze zbioru TSPLIB) w wersji do uruchomienia, znajdziesz na [OnlineGDB](https://onlinegdb.com/BNlHDeNXw). Możesz tam też zobaczyć, w jaki sposób rośnie liczba iteracji i czas wykonania wraz ze zwiększaniem się rozmiarów problemu.
...
Algorytm Helda-Karpa wykorzystuje programowanie dynamiczne, co oznacza, że będziemy najpierw rozwiązywać mały podproblem i na jego bazie budować rozwiązanie większego problemu. Co prawda nigdy nie opisywałem na blogu samego programowania dynamicznego, ale algorytmy na nim bazujące pokazywałem m.in. przy okazji [mierzenia podobieństwa ciągów znaków (algorytm Wagnera-Fischera)](/post/podobienstwo-ciagow-znakow) i [szukania najkrótszych ścieżek w grafie (algorytm Floyda-Warshalla)](/post/szukanie-najkrotszych-sciezek-w-grafie).
...
Analogicznie jak poprzednio, kod w całości z możliwością uruchomienia znajdziesz na [OnlineGDB](https://onlinegdb.com/2wKqMW92T). Możesz porównać wyniki z poprzednią implementacją (korzysta z tego samego zbioru danych).
