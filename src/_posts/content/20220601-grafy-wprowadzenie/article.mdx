...
export const meta = {
  ...
  coverCaption: (
    <>
      (grafika na okładce: <a href="https://commons.wikimedia.org/wiki/File:Goats_in_an_argan_tree_Morocco.jpg" target="_blank" rel="noreferrer">Elena Tatiana Chis</a>, <a href="https://creativecommons.org/licenses/by-sa/4.0" target="_blank" rel="noreferrer">CC BY-SA 4.0</a>, via Wikimedia Commons)
    </>
  ),
};
...
Hierarchię zapisują także wywodzące się z grafów drzewiaste struktury danych. Miałem już okazję pokazać na blogu ich zastosowanie w sortowaniu ([sortowanie przez kopcowanie](/post/sortowanie-cz-4-sortowanie-przez-wybieranie/#kopce) oraz [sortowanie drzewiaste](/post/sortowanie-cz-7-inne-podejscia/#sortowanie-drzewiaste)), gdzie graf wyznacza kolejność następowania po sobie elementów. Przedstawiłem też pojęcie [drzewa stanu gry, omawiając algorytm minimax](/post/algorytmika-gier-kolko-i-krzyzyk/#algorytm-minimax) — tutaj również w pamięci trzymaliśmy drzewo.
