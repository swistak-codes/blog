...
export const meta = {
  ...
  coverCaption: (
    <>
      (zdjęcie na okładce: <a href="https://commons.wikimedia.org/wiki/File:HK_TST_%E5%B0%96%E6%B2%99%E5%92%80_Tsim_Sha_Tsui_%E5%BD%8C%E6%95%A6%E9%81%93_Nathan_Road_%E7%BE%8E%E9%BA%97%E8%8F%AF%E5%95%86%E5%A0%B4_MiraPlace_Shopping_mall_shop_toys_December_2020_SS2_02.jpg" target="_blank" rel="noreferrer">MingUsuam Kimz</a>, <a href="https://creativecommons.org/licenses/by-sa/4.0" target="_blank" rel="noreferrer">CC BY-SA 4.0</a>, via Wikimedia Commons)
    </>
  ),
};
...
Zacznijmy od tego, dlaczego jest to algorytm probabilistyczny i jest testem pseudopierwszości. Generalnie rzecz ujmując, liczby pseudopierwsze to takie liczby pierwsze, które spełniają pewne własności liczb pierwszych, ale liczbami pierwszymi nie są, np. spełniają warunki małego twierdzenia Fermata. Więcej opowiedziałem o tym [w poprzednim artykule](/post/duze-liczby-pierwsze).
...
Drugie równanie wynika z [definicji reszty z dzielenia](/post/dziwny-przypadek-reszty-z-dzielenia), według której $$-1 \mod{n} = n-1$$. Warto o tym pamiętać, ponieważ jak opowiadam w zalinkowanym artykule, języki programowania potrafią zwracać ujemne wartości dla operacji modulo, podczas gdy zgodnie z definicją powinny one być zawsze dodatnie.
...
Jednak możesz zadać pytanie — czy metody probabilistyczne, gdzie nie jesteśmy w 100% pewni rozwiązania, są na pewno dobre? Otóż w większości zastosowań tak. Przypomnij sobie tylko moje wcześniejsze wpisy: [o całkach oznaczonych](/post/obliczanie-calek-oznaczonych) oraz [o sztucznej inteligencji](/post/sztuczna-inteligencja-a-co-to-a-komu-to-potrzebne) (dokładniej: akapit o rozwiązywaniu problemów). Zaznaczyłem tam, że w wielu zastosowaniach wystarczy nam przybliżone rozwiązanie problemu, a nie jego dokładny wynik. To samo jest tutaj. Jeśli zależy nam na szybkości, możemy zaryzykować, że wygenerujemy liczbę, która będzie złożona (aczkolwiek będzie silną pseudopierwszą). Z racji tego, jak test Millera-Rabina działa, będzie to mieć zdecydowanie lepszy rezultat, z większą szansą bycia liczbą pierwszą, niż pierwsza lepsza, całkowicie losowa liczba.
