...
export const meta = {
  ...
  coverCaption: (
    <>
      (oryginał zdjęcia na okładce: <a href="https://commons.wikimedia.org/wiki/File:Black-tailed_prairie_dogs_at_SF_Zoo_7.JPG" target="_blank" rel="noreferrer">BrokenSphere</a>, <a href="https://creativecommons.org/licenses/by/3.0" target="_blank" rel="noreferrer">CC BY 3.0</a>, via Wikimedia Commons)
    </>
  ),
};
...
Wszystko brzmi bardzo łatwo i przyjemnie, jednak w informatyce nie stosujemy [liczb dziesiętnych, tylko binarne](/post/matematyka-zero-jedynkowa). Jednak znając sposób działania algorytmu, zmiana myślenia na inny system liczbowy powinna być tutaj tylko formalnością. Natomiast to, co chciałem tu pokazać, to charakterystyczne cechy, które mogą Ci się przydać podczas implementacji tego algorytmu.
