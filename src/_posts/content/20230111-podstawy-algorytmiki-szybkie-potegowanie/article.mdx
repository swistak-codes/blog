...
Bazując na powyższej wiedzy, moglibyśmy zrobić prosty, teoretycznie prawidłowy algorytm potęgowania (kod dostępny na [OnlineGDB](https://onlinegdb.com/z_7q11D0T)):
...
Fani [rekurencji](/post/rekurencja-co-to-jest) mogliby zapisać go tak (kod dostępny na [OnlineGDB](https://onlinegdb.com/aMWrniruh)):
...
Skoro rekurencja, to dlaczego nie rekursja ogonowa? Co prawda w pokazywanym tutaj JavaScript nie ma sensu ten rodzaj optymalizacji ([przynajmniej na koniec 2022 roku](https://bugs.chromium.org/p/v8/issues/detail?id=4698)), ale mimo to pokażę (kod dostępny na [OnlineGDB](https://onlinegdb.com/cbkkiZIiL)):
...
Kod wraz z testami dostępny jest na [OnlineGDB](https://onlinegdb.com/y8iifQBW4).
...
Ten kod również znajdziesz na [OnlineGDB](https://onlinegdb.com/U-HgPy6dpX).

Oczywiście według definicji rekurencyjnej możemy napisać również kod korzystający ze zwykłej [iteracji](/post/iteracja-co-to-jest). Będzie to wyglądać następująco (dodałem komentarze tłumaczące, co się dzieje w odniesieniu do wersji rekurencyjnej):
...
Również kod tego rozwiązania zamieściłem na [OnlineGDB](https://onlinegdb.com/rbVGcXH0p). Tutaj oczywiście odniosłem się do pierwszej pokazanej przeze mnie definicji rekurencyjnej, ale możesz też spróbować w analogiczny sposób [zderekursywować](/post/derekursywacja) drugie podejście rekurencyjne. To już jednak pozostawiam Ci do zrobienia na własną rękę w ramach ćwiczenia. Jeśli potrzebujesz podpowiedzi, opis algorytmu można znaleźć w tomie drugim *Sztuki Programowania* D. Knutha.
...
Dla wygody będziemy iterować od tyłu, co możemy zrobić, bo mnożenie jest przemienne. Kod będzie wyglądać wówczas następująco (dostępny na [OnlineGDB](https://onlinegdb.com/k-72n05vh)):
...
Dodam też, że na bazie powyższego algorytmu powstał algorytm potęgowania modularnego, który opisałem w artykule *[Szybkie szukanie dużych liczb pierwszych](/post/szybkie-szukanie-duzych-liczb-pierwszych/#potęgowanie-modularne)*. Jeśli spojrzysz na przedstawiony tam kod, robimy dokładnie to samo, tylko z tą różnicą, że przy mnożeniach wykonujemy dodatkowo operację modulo.

## Potęgowanie liczby 2

W tym momencie warto wspomnieć, że gdy chcemy podnosić do potęgi liczbę 2, istnieje do tego o wiele lepszy, jeszcze wydajniejszy sposób, mianowicie przesunięcie bitowe w lewo. Opisałem to już w artykule *[1 0 0 0? 0 1 0 1! 1 0 0 1 – czyli matematyka zero-jedynkowa](/post/matematyka-zero-jedynkowa/#przesunięcia-bitowe)*, ale dla przypomnienia powtórzę tutaj w kontekście potęgowania.
...
Łącząc tę wiedzę w jedno, możemy napisać bardzo prostą funkcję do obliczania potęg dwójki (kod dostępny na [OnlineGDB](https://onlinegdb.com/ybS7vDyLT)):
