...
- [podstawowe pojęcia i rachunek zdań](/post/podstawy-logiki),
- [kwantyfikatory](/post/kwantyfikatory),
- [algebrę zbiorów](/post/algebra-zbiorow).
...
TypeScript z racji tego, że musi opisać słabo typowany język, jakim jest JavaScript, daje bardzo dużą dowolność w definiowaniu typów. Dzięki temu możemy pokryć najdziwniejsze przypadki, do których zmuszają nas sytuacje w kodzie aplikacji, a których nie spotkalibyśmy w językach programowania takich jak C++, C# czy Java. Pominę opis, po co to robić, ponieważ wiele osób się już na ten temat wypowiedziało w Internecie (w tym ja, sprawdź [Publikacje](/publikacje)). W kontekście artykułu interesuje nas, co system typów ma wspólnego ze zbiorami, albo nawet ogólniej — z logiką matematyczną.
