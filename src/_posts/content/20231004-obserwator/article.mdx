...
Poniżej możesz zobaczyć przykładową implementację wzorca obserwator w TypeScripcie. Zastosowałem ten język, ponieważ miałem okazję [omówić go ostatnio na blogu](/post/teoria-zbiorow-typescript), a także łatwo go będzie przenieść na „żywy” przykład. Dodatkowo poniżej zamieszczę ten sam kod przełożony na Kotlina, aby pokazać, że w innych językach wygląda to dokładnie tak samo (różnice wynikają jedynie ze składni i działania systemu typów).
...
Kod możesz przetestować na [OnlineGDB](https://onlinegdb.com/DqizAXkVaN). Dodatkowo na [tym OnlineGDB](https://onlinegdb.com/SNZ2kf2Sm) udostępniam obiecaną wcześniej wersję w Kotlinie.
...
Kod do przetestowania na własnym komputerze możesz ściągnąć [z GitHuba](https://github.com/swistak-codes/snippets-backup/tree/main/rxjs). Tak jak poprzednio, ten także przełożyłem na Kotlina z użyciem RxJava, co znajdziesz na [GitHubie](https://github.com/swistak-codes/snippets-backup/tree/main/rxjava). Również przerobiłem przykład na [StackBlitz](https://stackblitz.com/edit/observer-rxjs?file=src%2Fmain.ts), aby pokazać RxJS w swoim *naturalnym* środowisku.
...
Tradycyjnie kod możesz przetestować na [OnlineGDB](https://onlinegdb.com/R6mqgCfKB), a także jego webową wersję na [StackBlitz](https://stackblitz.com/edit/observer-signal?file=src%2Fmain.ts).
