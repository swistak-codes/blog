...
export const meta = {
  ...
  coverCaption: (
    <>
      (źródło zdjęcia na okładce: <a href="https://pxhere.com/en/photo/1042324" target="_blank" rel="noreferrer">pxhere</a>)
    </>
  ),
};
...
W artykule o [sortowaniu przez wybieranie](/post/sortowanie-cz-4-sortowanie-przez-wybieranie) omówiłem pokrótce struktury drzewiaste. Tam wykorzystaliśmy kopiec, który umożliwiał szybkie znalezienie największego elementu. Struktury tego typu możemy również inaczej wykorzystać w sortowaniu — mianowicie do samego posortowania elementów. Przykładem takiego rozwiązania jest algorytm sortowania drzewiastego. Wykorzystuje on strukturę zwaną binarnym drzewem poszukiwań (często powszechnie nazywana drzewem binarnym, jednak jest to określenie wprowadzające w błąd).

### Binarne drzewo poszukiwań

Binarne drzewo poszukiwań (z ang. binary search tree, BST) to bardzo ciekawa struktura danych. Jest to, jak nazwa wskazuje, drzewo, gdzie każdy element ma maksymalnie dwójkę dzieci i mamy dość ściśle wyznaczoną hierarchię. Lewe dziecko jest zawsze mniejsze od rodzica, natomiast prawe dziecko jest od niego większe. Hierarchia taka utrzymuje się przez całe drzewo. Tę ideę możesz znać z [wyszukiwania binarnego](/post/wyszukiwanie-w-listach) — właśnie na nim bazuje BST.
