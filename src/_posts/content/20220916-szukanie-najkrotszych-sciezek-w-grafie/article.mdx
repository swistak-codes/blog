...
export const meta = {
  ...
  coverCaption: (
    <>
      (zdjęcie na okładce: Image by <a href="https://pixabay.com/users/jemkaye-31694/?utm_source=link-attribution&utm_medium=referral&utm_campaign=image&utm_content=5452698" target="_blank" rel="noreferrer">Jayne</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=image&utm_content=5452698" target="_blank" rel="noreferrer">Pixabay</a>)
    </>
  ),
};
...
Samo wypisanie najkrótszych ścieżek wygląda analogicznie jak w przypadku szukania ich algorytmem BFS, co pokazywałem [w poprzednim artykule](/post/praktyczne-zastosowania-przechodzenia-po-grafie).
...
Przykładową implementację bazującą na macierzy sąsiedztwa znajdziesz [pod tym linkiem w serwisie OnlineGDB](https://onlinegdb.com/kRGU3N8GP). Implementacja zawiera graf widoczny na poniższym rysunku i szuka ścieżek do każdego wierzchołka od wierzchołka 0. Możesz spróbować przerobić graf i np. dodać do niego cykl, zmieniając wagę krawędzi `(5,3)` na -3.
...
<Image
  ...
  alt="Graf"
  caption={<>
    Graf użyty w implementacji algorytmu znajdującej się <a href="https://onlinegdb.com/kRGU3N8GP">tutaj</a>.
  </>}
/>
...
Ponownie wracając myślami do sortowania, przypomnij sobie algorytm [sortowania przez kopcowanie](/post/sortowanie-cz-4-sortowanie-przez-wybieranie/). Stosowaliśmy tam specjalną, dodatkową strukturę danych (kopiec), która umożliwiała szybkie znalezienie największego bądź najmniejszego elementu. Podobnie jest tutaj. Niekoniecznie musimy zastosować kopiec, tylko ogólnie mówiąc — kolejkę priorytetową (dokładniej — typu min).
...
Analogicznie, przykładową implementację bazującą na macierzy sąsiedztwa znajdziesz [pod tym linkiem w serwisie OnlineGDB](https://onlinegdb.com/awJE8eSPR). Implementacja zawiera graf, który możesz zobaczyć na poniższym rysunku, i szuka ścieżek do każdego wierzchołka od wierzchołka 0.
...
<Image
  ...
  alt="Graf"
  caption={<>
    Graf użyty w implementacji algorytmu znajdującej się <a href="https://onlinegdb.com/awJE8eSPR">tutaj</a>.
  </>}
/>
...
Analogicznie, przykładową implementację bazującą na macierzy sąsiedztwa znajdziesz [pod tym linkiem w serwisie OnlineGDB](https://onlinegdb.com/0SajmkKDo). Implementacja opiera się na tym samym grafie, który użyłem do zaprezentowania algorytmu Bellmana-Forda, więc nie wstawiam rysunku ponownie. Z racji tego, że dostajemy wszystkie ścieżki, w powyższej implementacji wypisuję ścieżki dla wszystkich par różnych wierzchołków.
...
- Jeśli graf jest nieważony, to nie używamy żadnego z powyższych algorytmów, tylko korzystamy z BFS, tak jak to opisałem [w poprzednim artykule](/post/praktyczne-zastosowania-przechodzenia-po-grafie). Algorytm ma złożoność czasową $$\Omicron(|E|+|V|)$$.
