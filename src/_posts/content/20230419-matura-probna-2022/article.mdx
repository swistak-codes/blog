...
export const meta = {
  ...
  updateTime: '2026-03-16T07:00:00+00:00',
...
- Mamy informację, że tekst składa się z 10000 znaków, ale w Javie czy Pythonie nie będzie nas to interesować. Będziemy po prostu odczytywać string znak po znaku (metodą `charAt(indeks)`). Jedynie, co warto zrobić, to obcięcie białych znaków na przodzie i końcu, aby nie zaburzyły [iteracji](/post/iteracja-co-to-jest).
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/cwOWVt0j4l) (plik FirstTask.java) wraz z resztą podzadań.
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/cwOWVt0j4l) (plik SecondTask.java) wraz z resztą podzadań. Zamieściłem tam też jako zakomentowaną drugą wersję z pętlą `for`, abyś zdecydował(a) na własną rękę, co jest czytelniejsze i co wolisz napisać.
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/cwOWVt0j4l) (plik ThirdTask.java) wraz z resztą podzadań. W komentarzu możesz zobaczyć wersję zapisaną wyrażeniem regularnym, jeśli jesteś ciekaw(a), jak w ten sposób można rozwiązać to zadanie.

### Podsumowanie

Całość kodu znajdziesz w wersji do uruchomienia na [OnlineGDB](https://onlinegdb.com/cwOWVt0j4l). Odpowiedzi, które widzimy w konsoli, zgadzają się z odpowiedziami zarówno dla plików `mecz_przyklad.txt` (wyniki w treściach zadań), jak i `mecz.txt` (wyniki w arkuszu z odpowiedziami).
...
- Nieco mniej oczywiste, ale można zauważyć, że wzory $$2 \cdot x$$ i $$2 \cdot x + 1$$ to nic innego jak wzory na lewe i prawe dziecko w [kopcu binarnym](/post/sortowanie-cz-4-sortowanie-przez-wybieranie/#kopce), który jest szczególnym przypadkiem drzewa binarnego.
...
Załóżmy, że na maturze nie znasz konkretnych wzorów. Jak w takim razie to obliczyć? Otóż przypadek, który tutaj mamy, jest idealnie zbalansowanym drzewem binarnym, czyli takim, gdzie węzły rozkładają się równomiernie. Gdybyśmy mieli do czynienia z [drzewem przeszukiwań binarnych](/post/sortowanie-cz-7-inne-podejscia/#sortowanie-drzewiaste), oznaczałoby to, że znalezienie jakiegokolwiek elementu zajmowałoby tyle czasu, co dla idealnego przypadku według złożoności obliczeniowej tej struktury. A tym idealnym przypadkiem jest złożoność logarytmiczna.
...
Całość kodu znajdziesz na [OnlineGDB](https://onlinegdb.com/hs06WPNFT).
...
- *[Liczby pierwsze i proste sposoby na ich sprawdzanie](/post/liczby-pierwsze-i-proste-sposoby-na-ich-sprawdzanie)*
- *[Proste sposoby na znajdowanie liczb pierwszych](/post/proste-sposoby-na-znajdowanie-liczb-pierwszych)*
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/eA0XGQJnQ) (plik FirstTask.java) wraz z resztą podzadań.
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/eA0XGQJnQ) (plik SecondTask.java) wraz z resztą podzadań.
...
Kod możesz sprawdzić na [OnlineGDB](https://onlinegdb.com/eA0XGQJnQ) (plik ThirdTask.java) wraz z resztą podzadań.
...
Całość kodu znajdziesz w wersji do uruchomienia na [OnlineGDB](https://onlinegdb.com/eA0XGQJnQ). Odpowiedzi widoczne w konsoli zgadzają się dla plików `liczby_przyklad.txt` (wyniki w treściach zadań) i `liczby.txt` (wyniki w arkuszu z odpowiedziami).
...
Aby obliczyć działanie zapisane w systemie czwórkowym, najprościej jest po prostu zrobić to pod kreską. Należy jedynie pamiętać, że największa cyfra, którą możemy mieć, to nie 9 (jak w systemie dziesiętnym), tylko 3. Moje przykładowe obliczenie znajdziesz poniżej. Jeśli potrzebujesz wyjaśnień, to sposób obliczania w systemie dwójkowym zaprezentowałem w artykule *[1 0 0 0? 0 1 0 1! 1 0 0 1 — czyli matematyka zero-jedynkowa](/post/matematyka-zero-jedynkowa/#dodawanie-i-odejmowanie)* — w tym przypadku zamiast 0 i 1 dostępnymi dla Ciebie cyframi są 0, 1, 2 i 3.
