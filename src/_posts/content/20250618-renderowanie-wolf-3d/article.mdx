...
export const meta = {
  ...
  cover,
  coverCaption: <>Zdjęcie na okładce wygenerowane przez Sora.</>,
};
...
- *[Przekształcenia grafiki 2D — matematyczny punkt widzenia](/post/przeksztalcenia-grafiki-2d)*
- *[Przekształcenia grafiki 3D](/post/przeksztalcenia-grafiki-3d)*
...
W kwestii planszy gry jako ciekawostkę dodam, że gdy przenoszono Wolfenstein 3D z komputerów na konsolę SNES, korzystanie z tak zapisanej planszy było niewydajne (dużo słabszy procesor). Na potrzeby portu przepisano plansze, aby korzystały z [drzew BSP](/post/wyszukiwanie-w-przestrzeni). Rozwiązanie to okazało się na tyle dobre, że twórcy zastosowali je w swojej kolejnej grze, czyli [Doomie](https://pl.wikipedia.org/wiki/Doom_(gra_komputerowa_1993)), i do dziś korzysta się z tego w silnikach 3D.
...
A w jaki sposób obliczyć pole widzenia? Do tego celu możemy użyć poniższego wzoru wykorzystującego fakt, że z pozycji gracza i rzutni możemy zbudować trójkąt równoramienny ([tutaj wyjaśnienie wzoru](/post/algorytmika-gier-obrot-do-punktu/#wyznaczenie-k%C4%85ta)):
...
Podstawą działania ray castingu jest algorytm Digital Differential Analyzer (DDA). Jest to algorytm pozwalający na [szybkie rysowanie linii](/post/jak-komputer-rysuje-linie) na siatce pikseli. W przypadku Wolfenstein 3D jest wykorzystywany do określenia, w który element trafia promień.
...
1. **Obliczanie parametrów algorytmu**
    1. **Różnica współrzędnych** — do określenia długości linii:
        - $$\Delta x = x_1 - x_0$$
        - $$\Delta y = y_1 - y_0$$
    2. **Liczba kroków do pokonania** — długość linii zmierzona w [metryce Czebyszewa](/post/szybkie-wyszukiwanie-sciezek/#gry):
...
W taki oto sposób uzyskujemy odległość bez funkcji trygonometrycznych, a także bez [pierwiastkowania](/post/pierwiastkowanie) i [podnoszenia do kwadratu](/post/szybkie-potegowanie) (odległość Euklidesowa). Jedyną „ciężką” operacją jest dzielenie przez wektor kierunku, ale to i tak jest znacznie prostsze niż pierwiastkowanie.
...
- **Sposób obliczeń** — z racji tego, że procesory [i386](https://en.wikipedia.org/wiki/I386), pod które była pisana gra, nie wspierały [obliczeń zmiennoprzecinkowych, gra korzystała z zapisu stałoprzecinkowego](/post/liczby-wymierne-i-rzeczywiste-w-zero-jedynkowym-swiecie) lub obliczeń na liczbach całkowitych (np. kąty zostały pomnożone przez 10 dla dokładniejszych obliczeń). Dzisiaj nie jest to problem, więc korzystamy z obliczeń zmiennoprzecinkowych. Zresztą [te w JavaScript są domyślne](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number) i to z użyciem liczb całkowitych mielibyśmy problem.
...
Jeśli nie wiesz, skąd się wzięły wzory na obrót wektora, możesz przeczytać o tym w artykule o [przekształcaniu grafiki 2D](/post/przeksztalcenia-grafiki-2d/#obr%C3%B3t). Tak, to właśnie tutaj jest ten moment, o którym wcześniej wspomniałem, że wiedza z tego artykułu się przyda.
