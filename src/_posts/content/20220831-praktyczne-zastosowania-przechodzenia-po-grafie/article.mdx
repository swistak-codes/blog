...
export const meta = {
  ...
  coverCaption: (
    <>
      (zdjęcie na okładce: Photo by form <a href="https://pxhere.com/en/photo/560162?utm_content=shareClip&utm_medium=referral&utm_source=pxhere" target="_blank" rel="noreferrer">PxHere</a> )
    </>
  ),
};
...
W idealnym świecie grafowa struktura danych przechowuje po prostu [graf](/post/grafy-wprowadzenie), czyli zbiór wierzchołków połączonych ze sobą krawędziami. W praktyce jednak może się niejednokrotnie zdarzyć, że wewnątrz jednej struktury znajduje się kilka grafów. W zasadzie każdy [sposób zapisu grafu w pamięci](/post/sposoby-reprezentacji-grafow) na to pozwala i jednocześnie nie zabrania takiej sytuacji.
...
Przykładową implementację bazującą na macierzy sąsiedztwa, napisaną w JavaScript znajdziesz [pod tym linkiem na serwisie OnlineGDB](https://onlinegdb.com/TZAhXnbGhD). W przykładzie zapisałem 5 grafów, ale możesz spróbować, jak algorytm zachowa się po zmodyfikowaniu macierzy. Weź jedynie pod uwagę fakt, że algorytm nie jest zabezpieczony na wypadek błędnie skonstruowanej struktury.
...
W tym przypadku naturalnym wyborem będzie algorytm DFS, ponieważ zwiedza on po kolei ścieżkę, a nie wykonuje się „warstwowo”. Tym samym, jeśli idąc ścieżką, trafimy ponownie na odwiedzony przez nas wierzchołek, sygnalizuje to istnienie cyklu. Przydatne tutaj będzie kolorowanie grafu, które opisałem [w poprzednim artykule](/post/przechodzenie-po-grafie), pisząc, że zwykle się go nie stosuje. W tym przypadku akurat jest przydatne. Przypomnijmy sobie na szybko te kolory:
...
Tak jak i poprzednio, przykładową implementację bazującą na macierzy sąsiedztwa, napisaną w JavaScript znajdziesz [pod tym linkiem na serwisie OnlineGDB](https://onlinegdb.com/adt2qB4SY). Graf, który tam zamieściłem, zawiera trzy cykle (w tym jeden spowodowany pętlą), ale zachęcam do kombinowania z innymi grafami.
...
Kroków algorytmu przechodzenia grafu rozpisywać nie będę, ponieważ jest to zwykły BFS, który już opisałem [w poprzednim artykule](/post/przechodzenie-po-grafie/#przeszukiwanie-wszerz). Jedyna różnica jest taka, że przed rozpoczęciem algorytmu tworzymy mapę poprzedników wierzchołków. Wtedy w samym algorytmie podczas odwiedzin wierzchołka zapisujemy, z którego wierzchołka trafiliśmy do niego. Warto pamiętać, żeby zapisanie poprzednika wykonać tylko raz. Jeśli trafimy na wierzchołek ponownie z innego, to trafiliśmy tam dłuższą drogą.
...
Ponownie, przykładową implementację bazującą na macierzy sąsiedztwa, napisaną w JavaScript znajdziesz [pod tym linkiem na serwisie OnlineGDB](https://onlinegdb.com/U-tXsZ6vx). Implementacja zawiera graf, który możesz zobaczyć na poniższym rysunku, i szuka ścieżek do każdego wierzchołka od wierzchołka 0.
...
<Image
  ...
  alt="Graf"
  caption={<>
    Graf użyty w implementacji algorytmu znajdującej się <a href="https://onlinegdb.com/U-tXsZ6vx">tutaj</a>.
  </>}
/>
