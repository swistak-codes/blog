...
Na potrzeby testów przepisałem algorytm na skrypt bashowy. Znajdziesz go na [OnlineGDB](https://onlinegdb.com/4HSW4OmrW). Jeśli nie masz Linuksa ani macOS, to możesz go uruchomić na OnlineGDB po wcześniejszym podaniu wartości w `Command line arguments`. Ewentualnie możesz skorzystać na Windowsie z [WSL](https://learn.microsoft.com/en-us/windows/wsl/).
...
Kod wraz z przykładem użycia znajdziesz na [OnlineGDB](https://onlinegdb.com/Y_ejr6joh). Zaznaczę tylko, że kod ten na pewno działa na Linuksie i powinien działać też na innych systemach zgodnych z POSIX (Uniksy, BSD, macOS...). Nie testowałem tego na Windowsie, ale zakładam, że z kompilatorem MSVC to nie zadziała, natomiast powinno zadziałać pod MinGW. Jeśli przetestowałeś(-aś), możesz dać mi znać w komentarzu (pod artykułem), czy i jak działa `clock_gettime`.
...
Kod znajdziesz na [OnlineGDB](https://onlinegdb.com/UL-jvYa-35), aczkolwiek jeśli uruchamiasz tam, to weź poprawkę na to, że kompilator C# działa tam dość wolno.
...
Kod jak zwykle znajdziesz na [OnlineGDB](https://onlinegdb.com/FR3iM-FzY). Jak widzisz, użycie jest analogiczne, różni się jedynie tym, którą funkcję wywołujemy. Warto tylko pamiętać o tym, że tutaj wynik jest typu `BigInt`.
...
Kod w całości znajdziesz na [OnlineGDB](https://onlinegdb.com/gWAqp8EVu).
...
Patrząc na powyższe fragmenty kodu, możesz zadać trafne pytanie — dlaczego po prostu nie odejmiemy od siebie daty po wykonaniu funkcji i przed wykonaniem? W końcu zwykle mamy dostęp do daty w postaci [czasu uniksowego](/post/jak-komputer-przechowuje-date-i-skad-zna-aktualna/#czas-uniksowy) w sekundach, a nawet milisekundach. Może to też być dla Ciebie o tyle zastanawiające, że często w artykułach skierowanych do początkujących programistów właśnie tak się odmierza czas.
