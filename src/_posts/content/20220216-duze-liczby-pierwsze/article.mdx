...
export const meta = {
  ...
  coverCaption: (
    <>
      (zdjęcie na okładce pochodzi z serwisu <a href="https://www.maxpixel.net/Museum-Skeleton-Dinosaur-Dinosaur-Skeleton-367732" target="_blank" rel="noreferrer">Max Pixel</a>)
    </>
  ),
};
...
Jednak jednocześnie to właśnie liczby Mersenne'a są największymi z liczb pierwszych, jakie znamy. Również są stosowane w informatyce (patrz np. algorytm Mersenne Twister). Dlaczego tak jest? Otóż są one najprostsze do przedstawienia [w formie binarnej](/post/matematyka-zero-jedynkowa). Każda liczba $$2^n-1$$ to w zapisie binarnym $$n$$ jedynek. Sprawia to też, że obliczenia na nich są stosunkowo proste.
